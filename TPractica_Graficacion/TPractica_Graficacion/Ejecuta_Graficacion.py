...
from TPractica_Graficacion import Plantilla_Grafica as grafica
...
class MyApp(QtWidgets.QMainWindow, grafica.Ui_MainWindow):
    def __init__(self):
        ...
        self.cb_Marcador.addItem("Marcador: x", "x")
        self.cb_Marcador.addItem("Marcador: 1", "1")
        self.cb_Marcador.addItem("Marcador: o", "o")
        self.cb_Marcador.addItem("Marcador: .", ".")
        self.cb_Marcador.currentIndexChanged.connect(self.Marcador)

        self.cb_ColorInterno.addItem("Verde", "green")
        self.cb_ColorInterno.addItem("Azul", "blue")
        self.cb_ColorInterno.addItem("Rojo", "red")
        self.cb_ColorInterno.addItem("Negro", "black")
        self.cb_ColorInterno.currentIndexChanged.connect(self.ColorInterno)

        self.cb_CBorde.addItem("Verde", "green")
        self.cb_CBorde.addItem("Azul", "blue")
        self.cb_CBorde.addItem("Rojo", "red")
        self.cb_CBorde.addItem("Negro", "black")
        self.cb_CBorde.currentIndexChanged.connect(self.CBorde)
        ...
        self.sp_TMarcador.setValue(1)
        self.sp_TMarcador.setMaximum(10)
        self.sp_TMarcador.setMinimum(1)
        self.sp_TMarcador.setSingleStep(1)
        self.sp_TMarcador.valueChanged.connect(self.TMarcador)

        self.sp_TBorde.setValue(1)
        self.sp_TBorde.setMaximum(10)
        self.sp_TBorde.setMinimum(1)
        self.sp_TBorde.setSingleStep(1)
        self.sp_TBorde.valueChanged.connect(self.TBorde)
    ...
    def TMarcador(self):
        tmarca=self.sp_TMarcador.value()
        self.TMarcador = tmarca
        self.limpiar()
        self.graficar()

    def TBorde(self):
        tborde=self.sp_TBorde.value();
        self.TBorde=tborde
        self.limpiar()
        self.graficar()

    def ColorInterno(self):
        colorint = self.cb_ColorInterno.currentData()
        self.ColorInterno = colorint
        self.limpiar()
        self.graficar()

    def CBorde(self):
        cborde = self.cb_CBorde.currentData()
        self.CBorde = cborde
        self.limpiar()
        self.graficar()

    def Marcador(self):
        marcador = self.cb_Marcador.currentData()
    ...
    def graficar(self):
        polinomio = self.txt_polinomio.text()  # 2x^2+4x+3
        polinomio = polinomio.replace("^", "**")  ##CAMBIAMOS SIMBOLO DE POTENCIA POR EL USADO EN PYTHON
        # 2x**2+4x+3

        # x = [i for i in range(-5,6)] #[-5 5]
        x = [i for i in range(self.xMin, self.xMax + 1)]  # GENERA LOS VALORES DE X
        print("Valores de X: ")
        print(x)
        ...
        self.ax.plot(x, y,
                     ...
                     marker=self.Marcador,  # o . *  x   1
                     markersize=self.TMarcador,
                     markerfacecolor=self.ColorInterno,  # color interno del marcador
                     markeredgewidth=self.TBorde,  # tamaño del borde del marcador
                     markeredgecolor=self.CBorde,  # color del borde del marcador
        ...
        # si comienzo con xmin en 5 seria:
        # xtick = []
        # for i in range(5, 85+1, 10):
        #    xtick.append(i)
        # print("Ticks para X: ")
        # print(xtick)
        #
        self.ytick.clear()
        self.sumay = 0
        for i in range(self.yDivisiones):
            self.sumay += int(self.yMax / self.yDivisiones)
            self.ytick.append(self.sumay)
        print(self.ytick)
        self.ax.set_yticks(self.ytick)
        ###
        self.xtick.clear()
        self.sumax=0
        for i in range(self.xDivisiones):
            self.sumax += int(self.xMax / self.xDivisiones)
            self.xtick.append(self.sumax)
        print(self.xtick)
        self.ax.set_xticks(self.xtick)
        # xtick = [5, 15, 25, 35, 45, 55, 65, 75, 85]
